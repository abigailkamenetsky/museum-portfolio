...
import { editor, useEditor, nudge, reset, exportJSON } from './paintingEditorStore'
...
export default function PaintingEditorPanel({ entries, placements }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === 'Escape') { editor.set({ selected: null }) }
    }
  ...
  return (
    <div style={box}>
      ...
      {!cur && <div style={{ opacity: .75 }}>Click a painting to select it.</div>}

      {cur && (
        <>
          <div style={{ color: '#f0d68a', marginBottom: 6 }}>
            {entries[i].title}
          </div>
